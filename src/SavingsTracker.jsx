...
export default function SavingsTracker() {
  ...
  const totalTarget = TARGET_PER_PERSON * (travellers.length || 1);
  ...
  const totalMonthlyNeeded = travellers.reduce((s, t) => {
    return s + Math.max(0, TARGET_PER_PERSON - (t.saved || 0)) / months;
  }, 0);
        ...
        {activeTab === "overview" && (
          <div>
            ...
            {travellers.length === 0 ? (
              ...
              const pct = (t.saved || 0) / TARGET_PER_PERSON;
              const remaining = Math.max(0, TARGET_PER_PERSON - (t.saved || 0));
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: "12px", color: "#445566" }}>{fmt(t.monthly_pledge || 0)}/mo</div>
                        </div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontWeight: "800", fontSize: "18px", color: t.color || "#4f8ef7" }}>{fmt(t.saved || 0)}</div>
                        <div style={{ fontSize: "11px", color: "#445566" }}>{Math.round((t.saved || 0) / TARGET_PER_PERSON * 100)}% of goal</div>
                      </div>
                    </div>
